...
class MapGenerationRequest(BaseModel):
    ...
    data_source: Optional[Literal["auto", "sentinel_hub", "opentopography", "bing_maps", "azure_maps", "google_earth_engine"]] = Field(
        "auto",
        description="Data source to use (auto=best available, sentinel_hub=free, opentopography=free DEM only, bing_maps=deprecated, azure_maps=requires key, google_earth_engine=requires setup)"
    )
